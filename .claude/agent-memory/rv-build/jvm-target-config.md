...
**JVM ceiling = 21 (hard).** The Gradle daemon is pinned to JDK 21 (`gradle/gradle-daemon-jvm.properties` → `toolchainVersion=21`) and CI provisions JDK 21 (Temurin, `.github/actions/gradle-setup/action.yml`). Kotlin/javac cannot emit bytecode newer than the compiling JDK, so 21 is the max regardless of what Android/Desktop accept. (Local launcher JVM is 25, but Gradle compiles on the daemon, not the launcher.) Android via AGP 9.1.0 / compileSdk 37 accepts 21 and is not the limiting factor.
